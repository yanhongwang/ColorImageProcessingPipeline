...
void __fastcall TForm1::TabControl1Change(TObject *Sender)
{
	...
	switch( TabControl1 -> TabIndex )
	{
		...
		case 1:
		{
			...
			char CTCFileName[ 150 ];
			strcpy( CTCFileName, FileListBox1 -> FileName.c_str() );
			
			sprintf( strrchr( CTCFileName, '.' ), "_ctc.bmp" );
			
			if( !FileExists( CTCFileName ) )
			{
				...
				/*
				// y = ax + b
				// Old Lctc : y = -1.0581x + 0.5801
				double slope = 1 / 1.0581;
				double b = y - slope * x;
				
				GRRatio = ( -b + 0.5801 ) / ( slope + 1.0581 );
				GBRatio = -1.0581 * GRRatio + 0.5801;
				*/
				...
				double RCoefficient = pow( 10, GRRatio );
				double BCoefficient = pow( 10, GBRatio );
				
				// Lctc : y = -1.0581x + 0.5801
				...
				for( int i = 0; i < imageHeight; i ++ )
					for( int j = 0; j < imageWidth; j ++ )
					{
						...
						// 1545 -> 1536, 2064 -> 2048
						//temp = BPlane[ i + 5 ][ j + 8 ] * 1.38;	// D65
						temp = ImageDataBlock[ i ][ 3 * j + 0 ] * BCoefficient;
						ImageDataBlock[ i ][ 3 * j + 0 ] = ( unsigned char )( temp < 0 ? 0 : temp >  255 ? 255 : temp );
						
						//temp = RPlane[ i + 5 ][ j + 8 ] * 2.17;	// D65
						temp = ImageDataBlock[ i ][ 3 * j + 2 ] * RCoefficient;
						ImageDataBlock[ i ][ 3 * j + 2 ] = ( unsigned char )( temp < 0 ? 0 : temp >  255 ? 255 : temp );
						
					}
				
				FILE *fp = fopen( CTCFileName, "wb" );
			...
			Image4 -> Picture -> LoadFromFile( CTCFileName );
			TabControl1->Width=Image4->Picture->Width;
			TabControl1->Height=Image4->Picture->Height;
