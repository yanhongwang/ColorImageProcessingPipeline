...
void __fastcall TForm1::TabControl1Change(TObject *Sender)
{
	...
	switch( TabControl1 -> TabIndex )
	{
		...
		case 1:
		{
			...
			char BlackLevelStractionFileName[ 150 ];
			
			strcpy( BlackLevelStractionFileName, FileListBox1 -> FileName.c_str() );
			
			sprintf( strrchr( BlackLevelStractionFileName, '.' ), "_BS.bmp" );
			
			if( !FileExists( BlackLevelStractionFileName ) )
			{
				
				
				ReadBMPData( FileListBox1 -> FileName.c_str() );
				
				int RBlackLevelThreshold = 0;
				int GBlackLevelThreshold = 0;
				int BBlackLevelThreshold = 0;
				
				BlackLevelThreshold( ImageDataBlock, imageHeight, imageWidth, &RBlackLevelThreshold, &GBlackLevelThreshold, &BBlackLevelThreshold );
				...
				for( int i = 0; i < imageHeight; i ++ )
					for( int j = 0; j < imageWidth * 3; j += 3 )
					{
						...
						// B channel
						if( ImageDataBlock[ i ][ j + 0 ] > BBlackLevelThreshold )
							ImageDataBlock[ i ][ j + 0 ] -= BBlackLevelThreshold;
						else
							ImageDataBlock[ i ][ j + 0 ] = 0;
						
						// G channel
						if( ImageDataBlock[ i ][ j + 1 ] > GBlackLevelThreshold )
							ImageDataBlock[ i ][ j + 1 ] -= GBlackLevelThreshold;
						else
							ImageDataBlock[ i ][ j + 1 ] = 0;
						
						// R channel
						if( ImageDataBlock[ i ][ j + 2 ] > RBlackLevelThreshold )
							ImageDataBlock[ i ][ j + 2 ] -= RBlackLevelThreshold;
						else
							ImageDataBlock[ i ][ j + 2 ] = 0;
						
					}
				
				FILE *fp = fopen( BlackLevelStractionFileName, "wb" );
			...
			TabControl1 -> Tabs -> Strings[ TabControl1 -> TabIndex ] = ExtractFileName( BlackLevelStractionFileName );
			Image4 -> Picture -> LoadFromFile( BlackLevelStractionFileName );
			TabControl1->Width=Image4->Picture->Width;
			TabControl1->Height=Image4->Picture->Height;
